...
def admin_signup_view(request):
    ...
    return render(request,'exam/admin_signup.html',context=mydict)
...
def generate_marksheet_pdf(request, student_id):
    ...
    student = SMODEL.Student.objects.get(pk=student_id)
    results = models.Result.objects.filter(student=student)

    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="{student.user}_marksheet.pdf"'
    ...
    pdf.drawString(150, 700, f"Student Name: {student.user}")
   
    # Loop through the exam results and add them to the PDF
    y = 600
    for result in results:
        pdf.drawString(150, 650, f"Course: {result.exam}")
        pdf.drawString(150, y, f"Date: {result.date}")
        pdf.drawString(150, 550, f"Marks: {result.marks}")
